...
if __name__ == '__main__':
        ...
        if lab != 'DontCare':
            ...
            def line(p1, p2, front=1):
                plt.gca().add_line(Line2D(
                    (p1[0], p2[0]), (p1[1], p2[1]), color=colors[names.index(lab) * 2 + front]))

            # draw the upper 4 horizontal lines
            # front = 0 for the front lines
            line(corners_img[0], corners_img[1], 0)
    ...
    plt.axis('off')
    plt.tight_layout()
    plt.savefig('kitti_3dbox_to_img.png', bbox_inches ="tight",dpi=300)
    plt.show()
